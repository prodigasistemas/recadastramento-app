...
public class ServicosTab extends Fragment {
	...
	public void instanciate() {
		...
		// Spinner Tipo de Ligação de água
		String descricaoLigacaoAgua = Controlador.getInstancia().getCadastroDataManipulator().selectDescricaoByCodigoFromTable(
				Constantes.TABLE_SITUACAO_LIGACAO_AGUA, String.valueOf(getServicos().getTipoLigacaoAgua()));
		
		if (getImovel().getImovelStatus() != Constantes.IMOVEL_A_SALVAR && descricaoLigacaoAgua != null) {
		...
		adapter = new ArrayAdapter(getActivity(), android.R.layout.simple_spinner_item, Controlador.getInstancia()
				.getCadastroDataManipulator().selectDescricoesFromTable(Constantes.TABLE_SITUACAO_LIGACAO_ESGOTO));
		adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
		...
		String descricaoLigacaoEsgoto = Controlador.getInstancia().getCadastroDataManipulator()
				.selectDescricaoByCodigoFromTable(Constantes.TABLE_SITUACAO_LIGACAO_ESGOTO, String.valueOf(getServicos().getTipoLigacaoEsgoto()));
		...
		String descricaoLocalInstalacaoRamal = Controlador.getInstancia().getCadastroDataManipulator()
				.selectDescricaoByCodigoFromTable(Constantes.TABLE_LOCAL_INSTALACAO_RAMAL, String.valueOf(getServicos().getLocalInstalacaoRamal()));
...
//		if (codigo != null && !codigo.trim().equals("0") && !codigo.trim().equals("") && !codigo.equals(String.valueOf(Constantes.NULO_INT)))
		...
		codigo = Controlador.getInstancia().getCadastroDataManipulator().selectCodigoByDescricaoFromTable(
				Constantes.TABLE_SITUACAO_LIGACAO_ESGOTO, ((Spinner) view.findViewById(R.id.spinnerLigacaoEsgoto)).getSelectedItem().toString());
		getServicos().setTipoLigacaoEsgoto(codigo);

		codigo = Controlador.getInstancia().getCadastroDataManipulator().selectCodigoByDescricaoFromTable(Constantes.TABLE_LOCAL_INSTALACAO_RAMAL,
				((Spinner) view.findViewById(R.id.spinnerLocalizacaoPontoServico)).getSelectedItem().toString());
