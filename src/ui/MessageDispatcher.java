...
public class MessageDispatcher {
	...
	public void enviarMensagem() {
		...
		synchronized (mensagem) {
			...
			try {
				...
				conexao.setRequestProperty("Content-Length", Integer.toString(this.mensagem.length));
				conexao.setRequestMethod("POST");

				conexao.setRequestProperty("User-Agent", "Profile/Android Apache");
				...
				if (rc == HttpURLConnection.HTTP_OK) {
					...
					// Agora, verificamos se o servidor nos enviou mensagem de
					// processamento ok
					...
					// Se foi ok, caso o servidor tenha enviado o caracter
					// esperado
	...
	public boolean controlarParametros(StringBuffer buffer, char caracter, StringBuffer bufferValorParametro, boolean ultimoCaracter) {
		...
		// Mensagem
		if ((buffer.toString().indexOf(PARAMETRO_MENSAGEM) > -1 && buffer.toString().length() > PARAMETRO_MENSAGEM.length()) ||
		// Imoveis a revisitar
				(buffer.toString().indexOf(PARAMETRO_IMOVEIS_PARA_REVISITAR) > -1 && buffer.toString().length() > PARAMETRO_IMOVEIS_PARA_REVISITAR.length()) ||
				// Tipo de arquivo carregado
				(buffer.toString().indexOf(PARAMETRO_TIPO_ARQUIVO) > -1 && buffer.toString().length() > PARAMETRO_TIPO_ARQUIVO.length())) {
			...
			if (caracter == CARACTER_FIM_PARAMETRO || ultimoCaracter) {
				...
				if (buffer.toString().indexOf(PARAMETRO_MENSAGEM) > -1) {
					...
					// Configuracao.getInstancia().setMatriculasRevisitar(
					// bufferValorParametro.toString() );
